...
const BACKTRACK_COLOR = "#ff9966"; // New color for backtracking
...
let visitedCells = [];
let solutionPath = [];
let currentPath = []; // Track current path for visualization
let solving = false;
let animationDelay = 50; // ms between animation steps
...
function generateMaze() {
  ...
  // Start the recursive carving process
...
function carvePassages(r, c) {
  // Directions: [r, c] - top, right, bottom, left
  const directions = [
    [-2, 0],
    [0, 2],
    [2, 0],
  ...
  for (let [dr, dc] of directions) {
    ...
    if (
      ...
      // Carve a passage
      maze[r + dr / 2][c + dc / 2] = 0; // The cell between current and new
      maze[newR][newC] = 0; // The new cell
